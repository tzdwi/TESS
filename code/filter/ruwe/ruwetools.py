...
class U0Interpolator():
    ...
    def __init__(self, gbprptable, gtable):
        """
        ...
        gbprptable : string
            Full path to the file table_u0_g_col.txt (including the file name)
        gtable : string
            Full path to the file table_u0_g.txt (including the file name)
        """
        ...
        u0data = np.genfromtxt(gbprptable, names=['g_mag', 'bp_rp', 'u0'], skip_header=1, delimiter=',')
        ...
        #self.gbprpinterpolator = interp2d(gmag, bprp, u0mesh, bounds_error=True)
        self.gbprpinterpolator = RectBivariateSpline(gmag, bprp, u0mesh, kx=1, ky=1, s=0)

        u0data = np.genfromtxt(gtable, names=['g_mag', 'u0'], skip_header=1, delimiter=',')
